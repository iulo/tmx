...
import {ref} from 'vue'
import logoUrl from '../assets/tmexclude.png'
...
<template>
  <div class="tmx-shell">
    <aside class="tmx-sidebar" :class="{compact: compactSidebar}" :style="{width: compactSidebar ? '72px' : '210px'}">
      <div class="tmx-brand"><img class="tmx-logo" :src="logoUrl" alt="tmx" /><div v-if="!compactSidebar" class="brand-copy"><div class="brand-title"><strong>tmx</strong><el-button class="sidebar-toggle" text circle :icon="Fold" @click="compactSidebar = true" /></div><small>Time Machine Exclude</small></div><el-button v-else class="sidebar-toggle compact-toggle" text circle :icon="Expand" @click="compactSidebar = false" /></div>
