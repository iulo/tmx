...
// @ts-ignore
import icon from "../assets/tmx.png";
...
export const About = () => {
  ...
  const openAck = async () => {
    const WebviewWindow = await import("@tauri-apps/api/webviewWindow").then(window => window.WebviewWindow);
    const target = await WebviewWindow.getByLabel("ack");
    await target?.show();
  }

  const openLicense = async () => {
    const WebviewWindow = await import("@tauri-apps/api/webviewWindow").then(window => window.WebviewWindow);
    const target = await WebviewWindow.getByLabel("license");
    await target?.show();
  }
    ...
    <>
      ...
      <Container pl={24} pt={12}>
        <Group gap={"xl"} align={"flex-start"} justify={"center"}>
          ...
          <Stack gap={"xs"}>
            <Stack gap={0}>
              <Title order={4}>tmx</Title>
              <Text size={"xs"}>{t('build', {version: data?.version ?? "unknown", date})}</Text>
            </Stack>
            <Divider/>
            <Text size={"xs"}>
              Original project: <A href={"https://github.com/PhotonQuantum/tmexclude"}>
