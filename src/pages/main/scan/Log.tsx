...
import {IconAlertTriangle, IconChevronLeft, IconHomeExclamation} from "@tabler/icons-react";
    ...
    <motion.div key={"done"} style={{height: "100%"}} {...fadeAnimation}>
      <Stack py={"xl"} sx={{height: "100%"}}>
        ...
        <Group sx={{width: "100%"}} justify={"center"}>
          ...
          <Stack sx={{width: 320}}>
            <Text fz={24}>{t('apply_log')}</Text>
            <ScrollArea.Autosize mah={200} styles={{root: {borderStyle: "hidden"}}} offsetScrollbars>
              {
                Object.entries(applyErrors?.errors ?? {}).map(([path, reason]) => (
                  <Group key={path} align={"center"} gap={"xs"} justify={"space-between"} sx={{minHeight: 50}}>
                    <PathText withinPortal keepFirst={3} keepLast={1} path={path} lineClamp={1}/>
                    <Tooltip multiline w={300} label={reason} withinPortal withArrow arrowSize={12}>
                      <Group gap={"xs"} ml={"auto"}>
                        <Text c={"dimmed"}>{t('failed')}</Text>
